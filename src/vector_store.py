...
class FaissVectorStore:
    ...
    def add_embeddings(self, embeddings: np.ndarray, metadatas: List[Any] = None):
        ...
        if self.index is None:
            self.index = faiss.IndexHNSW(dim) # -- > create index for the vector db 
...
if __name__ == "__main__":
    ...
    docs = emb_pipe.dict_to_documents(processed_docs)
    
    store = FaissVectorStore()
    store.build_from_documents(docs)
    store.load()
    print(store.query("what is the price of the s&p 500 index in 2015?", top_k=3))